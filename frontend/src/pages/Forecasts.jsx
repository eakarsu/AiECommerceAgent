...
export const Forecasts = () => {
  ...
  const loadForecasts = async () => {
    try {
      const data = await get('/api/forecasts');
      setForecasts(data);
  ...
  const generateAiSummary = async (data) => {
    try {
      const result = await post('/api/ai/analyze', {
        type: 'forecasts_summary',
        data: {
          totalForecasts: data.length,
          totalPredictedSales: data.reduce((sum, f) => sum + (f.predictedSales || 0), 0),
          totalPredictedRevenue: data.reduce((sum, f) => sum + parseFloat(f.predictedRevenue || 0), 0),
          avgAccuracy: data.filter(f => f.accuracy).reduce((sum, f) => sum + parseFloat(f.accuracy), 0) / data.filter(f => f.accuracy).length || 0,
          categories: [...new Set(data.map(f => f.category))]
        }
      });
      setAiSummary(result.insight || result);
    } catch (e) {
      setAiSummary('AI analysis: Review forecast accuracy trends and adjust prediction models for better inventory planning.');
    }
  };
  ...
  const handleRowClick = async (f) => {
    ...
    try {
      ...
      setAiInsight('Consider seasonal factors and market conditions when planning inventory based on this forecast.');
    }
    setAiLoading(false);
  };
  const handleCreate = async (e) => { e.preventDefault(); try { await post('/api/forecasts', { ...newForecast, predictedSales: parseInt(newForecast.predictedSales), predictedRevenue: parseFloat(newForecast.predictedRevenue) }); setIsNewModalOpen(false); loadForecasts(); } catch (e) { console.error(e); } };
  const handleDelete = async () => { if (!window.confirm('Are you sure you want to delete this forecast?')) return; try { await del(`/api/forecasts/${selectedForecast.id}`); setIsModalOpen(false); loadForecasts(); } catch (e) { console.error(e); } };

  const columns = [
    { header: 'Forecast', render: (row) => (
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-fuchsia-100 rounded-lg flex items-center justify-center text-lg">🔮</div>
        <div><span className="font-medium block">{row.Product?.name || row.category || 'Category Forecast'}</span><span className="text-sm text-gray-500">{row.category}</span></div>
      </div>
    )},
  ...
  return (
    <div className="space-y-6">
      ...
      {aiSummary && (
        <div className="bg-gradient-to-r from-fuchsia-50 to-purple-50 border border-fuchsia-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            <span className="text-2xl">✨</span>
            <div><h3 className="font-medium text-fuchsia-900 mb-1">AI Forecast Insights</h3><p className="text-fuchsia-800 text-sm">{aiSummary}</p></div>
          </div>
        </div>
      )}
      ...
      <div className="card"><DataTable columns={columns} data={forecasts} onRowClick={handleRowClick} loading={loading} /></div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
                <p className="text-sm text-blue-600 mb-2">Predicted</p>
                <p className="text-3xl font-bold text-blue-700">{selectedForecast.predictedSales?.toLocaleString()}</p>
                <p className="text-lg text-blue-600">${selectedForecast.predictedRevenue?.toLocaleString()}</p>
              </div>
              <div className={`border rounded-lg p-6 text-center ${selectedForecast.actualSales ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
                ...
                {selectedForecast.actualSales ? (
                  <>
                    <p className="text-3xl font-bold text-green-700">{selectedForecast.actualSales?.toLocaleString()}</p>
                    <p className="text-lg text-green-600">${selectedForecast.actualRevenue?.toLocaleString()}</p>
                  </>
                ) : <p className="text-2xl text-gray-400">Pending</p>}
              </div>
            </div>
            {selectedForecast.accuracy && (
              <div><p className="text-sm text-gray-500 mb-2">Forecast Accuracy</p>
                <div className="flex items-center gap-3"><div className="flex-1 bg-gray-200 rounded-full h-4"><div className={`h-4 rounded-full ${selectedForecast.accuracy > 95 ? 'bg-green-500' : selectedForecast.accuracy > 90 ? 'bg-yellow-500' : 'bg-red-500'}`} style={{width: `${selectedForecast.accuracy}%`}}></div></div><span className="text-xl font-bold">{parseFloat(selectedForecast.accuracy).toFixed(1)}%</span></div>
              </div>
            )}
            {selectedForecast.confidenceInterval && (
              <div className="bg-gray-50 rounded-lg p-4">
                <p className="text-sm text-gray-500 mb-2">Confidence Interval</p>
                <p className="font-medium">Low: {selectedForecast.confidenceInterval.low?.toLocaleString()} - High: {selectedForecast.confidenceInterval.high?.toLocaleString()}</p>
              </div>
            )}
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => handleRowClick(selectedForecast)} disabled={aiLoading} className="text-sm text-purple-600 hover:text-purple-800 disabled:opacity-50">🔄 Regenerate</button>
              </div>
