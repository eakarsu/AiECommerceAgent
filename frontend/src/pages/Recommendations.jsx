...
export const Recommendations = () => {
    ...
    try {
      const result = await post('/api/ai/analyze', {
        ...
        data: {
          ...
          totalClicks: data.reduce((sum, r) => sum + (r.clicks || 0), 0),
          totalConversions: data.reduce((sum, r) => sum + (r.conversions || 0), 0),
          totalRevenue: data.reduce((sum, r) => sum + parseFloat(r.revenue || 0), 0),
      ...
      setAiSummary('AI analysis: High-score recommendations are driving revenue. Consider expanding successful cross-sell patterns.');
    }
  };
  ...
  const handleRowClick = async (r) => {
    ...
    try {
      const result = await post('/api/ai/analyze', {
        ...
        data: {
          type: r.type,
          sourceProduct: products.find(p => p.id === r.sourceProductId)?.name,
          targetProduct: products.find(p => p.id === r.targetProductId)?.name,
          score: r.score,
          clicks: r.clicks,
      ...
      setAiInsight('Analyze the conversion rate and consider adjusting positioning or timing of this recommendation.');
    }
    setAiLoading(false);
  };
  const handleCreate = async (e) => { e.preventDefault(); try { await post('/api/recommendations', { ...newRec, sourceProductId: parseInt(newRec.sourceProductId), targetProductId: parseInt(newRec.targetProductId), score: parseFloat(newRec.score) }); setIsNewModalOpen(false); loadRecommendations(); } catch (e) { console.error(e); } };
  const handleDelete = async () => { if (!window.confirm('Are you sure you want to delete this recommendation?')) return; try { await del(`/api/recommendations/${selectedRec.id}`); setIsModalOpen(false); loadRecommendations(); } catch (e) { console.error(e); } };

  const handleRecalculate = async () => {
    setRecalculating(true);
    try {
      await post('/api/recommendations/recalculate');
      loadRecommendations();
    } catch (e) { console.error(e); }
  ...
  const columns = [
    { header: 'Recommendation', render: (row) => (
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-amber-100 rounded-lg flex items-center justify-center text-lg">💡</div>
        <div><span className={`badge ${typeColors[row.type]}`}>{row.type.replace('_', ' ')}</span></div>
      </div>
    )},
    ...
    { header: 'Score', render: (row) => (
      <div className="flex items-center gap-2">
        <div className="w-12 bg-gray-200 rounded-full h-2"><div className="h-2 rounded-full bg-amber-500" style={{width: `${row.score * 100}%`}}></div></div>
        <span className="font-bold">{(parseFloat(row.score || 0) * 100).toFixed(0)}%</span>
      </div>
    )},
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={handleRecalculate} disabled={recalculating} className="btn btn-secondary">
      ...
      {aiSummary && (
        <div className="bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div><h3 className="font-medium text-amber-900 mb-1">AI Recommendation Insights</h3><p className="text-amber-800 text-sm">{aiSummary}</p></div>
          </div>
        </div>
      )}
      ...
      <div className="card space-y-4">
        <LiveSearch
          ...
          onFilter={setFilteredRecommendations}
          searchFields={['type', 'reason', 'status']}
          placeholder="Search by type, reason, status..."
        />
        <DataTable columns={columns} data={filteredRecommendations} onRowClick={handleRowClick} loading={loading} />
      </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Recommendation Details" size="lg">
        {selectedRec && (
          <div className="space-y-6">
            ...
            <div><p className="text-sm text-gray-500 mb-2">Recommendation Score</p>
              <div className="flex items-center gap-3"><div className="flex-1 bg-gray-200 rounded-full h-4"><div className="h-4 rounded-full bg-gradient-to-r from-amber-400 to-amber-600" style={{width: `${parseFloat(selectedRec.score || 0) * 100}%`}}></div></div><span className="text-xl font-bold">{(parseFloat(selectedRec.score || 0) * 100).toFixed(0)}%</span></div>
            </div>
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => handleRowClick(selectedRec)} disabled={aiLoading} className="text-sm text-purple-600 hover:text-purple-800 disabled:opacity-50">🔄 Regenerate</button>
              </div>
