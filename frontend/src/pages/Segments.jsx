...
export const Segments = () => {
    ...
    try {
      const result = await post('/api/ai/analyze', {
        ...
        data: {
          ...
          totalCustomers: data.reduce((sum, s) => sum + (s.customerCount || 0), 0),
          totalRevenue: data.reduce((sum, s) => sum + parseFloat(s.totalRevenue || 0), 0),
          avgChurn: data.reduce((sum, s) => sum + parseFloat(s.churnRate || 0), 0) / data.length,
          activeSegments: data.filter(s => s.status === 'active').length
        }
      });
      setAiSummary(result.insight || result);
    } catch (e) {
      setAiSummary('AI analysis: Focus retention efforts on high-value segments with elevated churn risk for maximum impact.');
    }
  };
  ...
  const handleRowClick = async (s) => {
    ...
    try {
      ...
      setAiInsight('Consider personalized engagement strategies for this segment to reduce churn and increase lifetime value.');
    }
    setAiLoading(false);
  };
  const handleAiAnalyze = async () => {
    ...
    try {
      const result = await post('/api/ai/analyze', {
        type: 'segment_deep_analysis',
        data: {
          name: selectedSegment.name,
          description: selectedSegment.description,
          customerCount: selectedSegment.customerCount,
          averageValue: selectedSegment.averageValue,
          totalRevenue: selectedSegment.totalRevenue,
          growthRate: selectedSegment.growthRate,
          churnRate: selectedSegment.churnRate,
          status: selectedSegment.status,
          requestType: 'Provide detailed segment analysis with personalized marketing strategies and retention recommendations'
        }
      });
      setAiInsight(result.insight || result);
    } catch (e) {
      setAiInsight('Deep analysis: Focus on personalized engagement, loyalty programs, and targeted campaigns to maximize this segment\'s lifetime value.');
    }
    setAiLoading(false);
  };
  const handleDelete = async () => { if (!window.confirm('Are you sure you want to delete this segment?')) return; try { await del(`/api/segments/${selectedSegment.id}`); setIsModalOpen(false); loadSegments(); } catch (e) { console.error(e); } };
  const handleCreate = async (e) => { e.preventDefault(); try { await post('/api/segments', { ...newSegment, criteria: JSON.parse(newSegment.criteria) }); setIsNewModalOpen(false); loadSegments(); } catch (e) { console.error(e); } };

  const columns = [
    { header: 'Segment', render: (row) => (
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-rose-100 rounded-lg flex items-center justify-center text-lg">🎯</div>
        <div><span className="font-medium block">{row.name}</span><span className="text-sm text-gray-500">{row.customerCount?.toLocaleString()} customers</span></div>
      </div>
    )},
  ...
  return (
    <div className="space-y-6">
      ...
      {aiSummary && (
        <div className="bg-gradient-to-r from-rose-50 to-pink-50 border border-rose-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div><h3 className="font-medium text-rose-900 mb-1">AI Segment Insights</h3><p className="text-rose-800 text-sm">{aiSummary}</p></div>
          </div>
        </div>
      )}
      ...
      <div className="card"><DataTable columns={columns} data={segments} onRowClick={handleRowClick} loading={loading} /></div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => handleRowClick(selectedSegment)} disabled={aiLoading} className="text-sm text-purple-600 hover:text-purple-800 disabled:opacity-50">🔄 Regenerate</button>
              </div>
