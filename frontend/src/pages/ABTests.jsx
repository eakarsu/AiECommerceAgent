...
export const ABTests = () => {
  ...
  const loadTests = async () => {
    try {
      const data = await get('/api/ab-tests');
      setTests(data);
  ...
  const generateAiSummary = async (data) => {
    try {
      const result = await post('/api/ai/analyze', {
        type: 'abtests_summary',
        data: {
          totalTests: data.length,
          runningTests: data.filter(t => t.status === 'running').length,
          completedTests: data.filter(t => t.status === 'completed').length,
          variantAWins: data.filter(t => t.winner === 'A').length,
          variantBWins: data.filter(t => t.winner === 'B').length,
          avgConfidence: data.filter(t => t.confidenceLevel).reduce((sum, t) => sum + parseFloat(t.confidenceLevel || 0), 0) / data.filter(t => t.confidenceLevel).length || 0
        }
      });
      setAiSummary(result.insight || result);
    } catch (e) {
      setAiSummary('AI analysis: Focus on tests with high confidence levels and implement winning variants to improve conversions.');
    }
  };
  ...
  const handleRowClick = async (t) => {
    ...
    try {
      const result = await post('/api/ai/analyze', {
        type: 'abtest_analysis',
        data: {
          name: t.name,
          type: t.type,
          variantAViews: t.variantAViews,
          variantAConversions: t.variantAConversions,
          variantBViews: t.variantBViews,
      ...
      setAiInsight('Analyze conversion rates and statistical significance before making decisions on this test.');
    }
    setAiLoading(false);
  };
  const handleAiAnalyze = async () => {
    ...
    try {
      const result = await post('/api/ai/analyze', {
        type: 'abtest_deep_analysis',
        data: {
          name: selectedTest.name,
          type: selectedTest.type,
          variantA: selectedTest.variantA,
          variantB: selectedTest.variantB,
          variantAViews: selectedTest.variantAViews,
          variantAConversions: selectedTest.variantAConversions,
          variantBViews: selectedTest.variantBViews,
          variantBConversions: selectedTest.variantBConversions,
          conversionRateA: selectedTest.variantAViews > 0 ? (selectedTest.variantAConversions / selectedTest.variantAViews * 100).toFixed(2) : 0,
          conversionRateB: selectedTest.variantBViews > 0 ? (selectedTest.variantBConversions / selectedTest.variantBViews * 100).toFixed(2) : 0,
          winner: selectedTest.winner,
          confidenceLevel: selectedTest.confidenceLevel,
          status: selectedTest.status,
          requestType: 'Provide a detailed statistical analysis with actionable recommendations for this A/B test'
        }
      });
      setAiInsight(result.insight || result);
    } catch (e) {
      setAiInsight('Deep analysis: Review both variants carefully. Consider sample size, statistical significance, and business impact before making decisions.');
    }
    setAiLoading(false);
  };
  const handleDelete = async () => { if (!window.confirm('Are you sure you want to delete this A/B test?')) return; try { await del(`/api/ab-tests/${selectedTest.id}`); setIsModalOpen(false); loadTests(); } catch (e) { console.error(e); } };
  const handleCreate = async (e) => { e.preventDefault(); try { await post('/api/ab-tests', { ...newTest, variantA: JSON.parse(newTest.variantA), variantB: JSON.parse(newTest.variantB), status: 'draft' }); setIsNewModalOpen(false); loadTests(); } catch (e) { console.error(e); } };

  const columns = [
    { header: 'Test', render: (row) => (
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-violet-100 rounded-lg flex items-center justify-center text-lg">🧪</div>
        <div><span className="font-medium block">{row.name}</span><span className="text-sm text-gray-500">{row.type}</span></div>
      </div>
    )},
  ...
  return (
    <div className="space-y-6">
      ...
      {aiSummary && (
        <div className="bg-gradient-to-r from-violet-50 to-purple-50 border border-violet-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div><h3 className="font-medium text-violet-900 mb-1">AI A/B Testing Insights</h3><p className="text-violet-800 text-sm">{aiSummary}</p></div>
          </div>
        </div>
      )}
      ...
      <div className="card"><DataTable columns={columns} data={tests} onRowClick={handleRowClick} loading={loading} /></div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className={`border-2 rounded-lg p-4 ${selectedTest.winner === 'A' ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
                ...
                <div className="space-y-2 text-sm">
                  <p>Views: <span className="font-bold">{selectedTest.variantAViews?.toLocaleString()}</span></p>
                  <p>Conversions: <span className="font-bold text-green-600">{selectedTest.variantAConversions?.toLocaleString()}</span></p>
                  <p>Conv. Rate: <span className="font-bold">{getConversionRate(selectedTest.variantAConversions, selectedTest.variantAViews)}%</span></p>
                </div>
              ...
              <div className={`border-2 rounded-lg p-4 ${selectedTest.winner === 'B' ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
                ...
                <div className="space-y-2 text-sm">
                  <p>Views: <span className="font-bold">{selectedTest.variantBViews?.toLocaleString()}</span></p>
                  <p>Conversions: <span className="font-bold text-green-600">{selectedTest.variantBConversions?.toLocaleString()}</span></p>
                  <p>Conv. Rate: <span className="font-bold">{getConversionRate(selectedTest.variantBConversions, selectedTest.variantBViews)}%</span></p>
                </div>
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => handleRowClick(selectedTest)} disabled={aiLoading} className="text-sm text-purple-600 hover:text-purple-800 disabled:opacity-50">🔄 Regenerate</button>
              </div>
